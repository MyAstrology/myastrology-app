...
for (const s of [...seen].sort()) {
  if (!s) continue;
  const hit = lookup(s);
  /* ⛔ চাবি সবসময় স্বাভাবিক রূপে — অ্যাপের কোডে য় কখনো
     ভাঙা (য + নুক্তা) রূপে থাকে, আর হুবহু লিখলে একই নাম দুইবার
     বসত ("বিজয়াদশমী", "মহালয়়া", "অক্ষয় তৃতীয়়া" — মেপে দেখা)।
     দুই কপি একদিন সরে যায়; লুকআপ দু'দিকেই nfc() করে, তাই
     স্বাভাবিক রূপে রাখলে কিছুই হারায় না। */
  if (hit) found[norm(s)] = hit;
  else missing.push(s);
}
