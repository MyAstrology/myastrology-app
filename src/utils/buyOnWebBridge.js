...
export function buildBuyOnWebJS(page) {
  ...
  (function(){
    ...
    /* ⚠️ আগে বাংলা বাক্যাংশটাও মেলাতে হতো। ইংরেজি/হিন্দি পাতায় ওই
       টোস্ট অনূদিত হয়ে আসে ("Please use the myastrology.in website to
       pay"), তাই শর্তটা মিলত না — en/hi পাঠক টোস্ট দেখতেন আর কেনার
       প্রস্তাবই উঠত না। ঠিকানাটা তিন ভাষাতেই এক, তাই সেটাই যথেষ্ট ও
       ভাষা-নিরপেক্ষ। */
    function isBuyMsg(m){
...
async function playBuy(product, inject, msg) {
  ...
  try {
    ...
    /* ⚠️ এই বার্তাটা সব ব্যর্থতায় একরকম দেখায়, তাই কোনটা আটকেছে
       ...
       অনুবাদ হয় না — ওটা যন্ত্রের লেখা, পাঠকের ভাষার নয়। */
    /* ⚠️ ২১/৯/২০২৬ — আগে কেবল `e.code` দেখানো হতো, তাই পর্দায়
       আসত কেবল [functions/permission-denied] — আর সার্ভার যে কারণটা
    ...
    Alert.alert(
      ...
      t('টাকা কাটা হয়ে থাকলে চিন্তা করবেন না — পরের বার অ্যাপ খুলে একই বোতামে চাপ দিলে নতুন করে টাকা লাগবে না। এখনই দরকার হলে ওয়েবসাইট থেকেও নেওয়া যাবে।') + (code ? '\n\n[' + code + ']' : ''),
      [
